...
function CollectionRepeatDirective($ionicCollectionManager, $parse, $window, $$rAF, $rootScope, $timeout) {
  return {
    controller: collectionRepeatController,
    restrict: 'A',
    priority: 1000,
    transclude: 'element',
    $$tlb: true,
    require: ['collectionRepeat', '^^$ionicScroll'],
    link: postLink
  };

  function collectionRepeatController($scope, $element, $attrs) {
  ...
  function postLink(scope, element, attr, ctrls, transclude) {
    ...
    selfCtrl.getPrimaryPos   = getRepeatManager().getPrimaryPos;
    selfCtrl.getSecondaryPos = getRepeatManager().getSecondaryPos;
  }
...
function RepeatManagerFactory($rootScope, $window, $$rAF) {
  ...
  return function RepeatController(options) {
    ...
    this.getPrimaryPos = function getPrimaryPos(index) {
      return view.getDimensions(index).primaryPos;
    };

    this.getSecondaryPos = function getSecondaryPos(index) {
      return view.getDimensions(index).secondaryPos;
    };
